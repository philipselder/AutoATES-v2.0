...
#     def thread_finished(self, z_delta, flux, count_array, z_delta_sum, backcalc, fp_ta, fp_dis):
...
def main(args, kwargs):
    ...
    res_dir = root_dir / 'cache' / 'ATES' / 'FlowPy'
    log_dir = root_dir / 'log'

    dem_path = args[0]
    release_path = args[1]

    alpha = float(args[2]) if len(args) > 2 else 25
    exp = float(args[3]) if len(args) > 3 else 8  # PSE note: as exp increases, the avalanche flow grows closer to a single flow direction. As it approaches 1 (min), wide spreading occurs for the avalanche runoff.

    if len(args) > 4:
        raise ValueError('Too many positional arguments. Usage: main.py <dem_path> <release_path> [alpha] [exp] [key=value ...]')
    ...
    logging.basicConfig(level=logging.INFO,
                        format='%(asctime)s %(levelname)-8s %(message)s',
                        datefmt='%Y-%m-%d %H:%M:%S',
                        filename=str(log_dir / 'log_{}.txt'.format(time_string)),
                        filemode='w')
    ...
    io.output_raster(dem_path,
                     str(res_dir / "flux{}".format(output_format)),
                     flux)
    io.output_raster(dem_path,
                     str(res_dir / "z_delta{}".format(output_format)),
                     z_delta)
    io.output_raster(dem_path,
                     str(res_dir / "FP_travel_angle{}".format(output_format)),
                     fp_ta)
    io.output_raster(dem_path,
                     str(res_dir / "SL_travel_angle{}".format(output_format)),
                     fp_dis)
    if not calc_bool:  # if no infra
        io.output_raster(dem_path,
                         str(res_dir / "cell_counts{}".format(output_format)),
                         cell_counts)
        io.output_raster(dem_path,
                         str(res_dir / "z_delta_sum{}".format(output_format)),
                         z_delta_sum)
    if calc_bool:  # if infra
        io.output_raster(dem_path,
                         str(res_dir / "backcalculation{}".format(output_format)),
                         backcalc)
...
if __name__ == '__main__':
    ...
    #argv = sys.argv[1:]
    argv = ["./examples/test/dem.tif", "./examples/test/pra2.tif", "23", "8", "forest=./examples/test/forest2.tif", "infra=./examples/test/infra.tif", "flux=0.003" ,  "max_z=270"] #"forest=../runs/open_slope/forest.tif",
    #argv = ['--gui']
    if len(argv) == 1 and argv[0] == '--gui':
        ...
        if len(args) < 2:
            ...
            print("Usage: python main.py <dem_path> <release_path> [alpha] [exp] [key=value ...]")
            sys.exit(1)

        main(args, kwargs)
    
# example dam: python3 main.py ./examples/dam/dam_010m_standard_cr100_sw250_f2500.20.6_n0.asc ./examples/dam/release_dam.tif
# example dam/infra: python3 main.py ./examples/dam/dam_010m_standard_cr100_sw250_f2500.20.6_n0.asc ./examples/dam/release_dam.tif 25 8 infra=./examples/dam/infra.tif flux=0.003 max_z=270
# example dam/forest: python3 main.py ./examples/dam/dam_010m_standard_cr100_sw250_f2500.20.6_n0.asc ./examples/dam/release_dam.tif 25 8 forest=./examples/dam/infra.tif flux=0.003 max_z=270
